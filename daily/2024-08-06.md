...
##### js 재정의 (오버라이딩?) #다른친구_질문사항

``` js
Student.prototype.display = function () {
	// call(this): __self__ 같은 느낌?
	People.prototype.display.call(this); // 주석처리 해버릴 수도 있긴하다
	console.log(this.grade, "학년");
};
```
Student 형의 객체가 display() 함수를 호출하는 경우 부모로 바로 가지 않고 자식부터 호출한다..
이런 것을 property shadowing 이라고 한다

##### 실습 : Dog, Cat 타입 Animal 상속시켜보기..
#보충하기


### 6교시 실습: 파칭코 만들기
부제 : li 태그 여러개를 담은 슬롯 9개 분신술시키기..
